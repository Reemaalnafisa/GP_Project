...
class AR_ParentDashboard extends StatefulWidget {
  final String childEmail; // بريد الطفل
  final String childName;  // اسم الطفل

  // Constructor لاستقبال القيم
  AR_ParentDashboard({required this.childEmail, required this.childName});

  @override
  _ParentDashboardState createState() => _ParentDashboardState();
}

class _ParentDashboardState extends State<AR_ParentDashboard> {
  ...
  Future<Map<int, double>> calculateActiveHours() async {
    ...
    final currentDay = today.weekday % 7; // الأحد = 0
        ...
        if (startTime != null && endTime != null) {
          if (endTime.isAfter(startTime)) {
            ...
            if (durationInHours > 24 || durationInHours < 0) {
              print("⚠️ Skipping invalid duration: $durationInHours hours for day ${startTime.weekday}");
              continue;
            }
            ...
            // تعديل حساب اليوم ليبدأ من الأحد
            final day = (startTime.weekday % 7); // الأحد يصبح 0
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // النص تحت الدائرة
          Text(
            "$passedQuizzes / $totalQuizzes اختبارات",
            style: TextStyle(
              fontSize: 14,
      ...
      child: Column(
        ...
        children: [
          ...
          currentData != null
            ...
            children: [
              Text(
                "الدرجة: ${isGroupSelected ? currentData?['playerDetails']?['score']?.toString() ?? 'N/A' : currentData?['score']?.toString() ?? 'N/A'}",
                style: TextStyle(fontSize: 14),
              ),
              Text(
                "التاريخ: ${isGroupSelected ? (currentData?['playerDetails']?['timestamp'] != null
...
class _BarGraphCardState extends State<BarGraphCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality( // إضافة التوجه من اليمين إلى اليسار
            ...
            Expanded(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final barMaxHeight = maxHeight * 0.6; // تحديد 60% كحد أقصى لارتفاع الأعمدة
